...
Manager.prototype.submitJob = function submitJob(jobid, data, opts, submitted_cb, completed_cb) {
    ...
    self.redis.eval(scripts.addJob, 7, 
                    ...
                    jobid, JSON.stringify(data), opts.priority || 0, Date.now(), opts.delay || 0, !!opts.nx ? 1:0, opts.autoremove === undefined ? -1 : opts.autoremove,
                    function(err, result) {
        if(err) {
...
Manager.prototype.removeJob = function removeJob(jobid, callback) {
    ...
    this.redis.eval(scripts.removeJob, 7,
                    this.queuekey, this.delayqueuekey, this.activesetkey, this.completesetkey, this.livesetkey, this.delayprioritieskeys, this.jobKey(jobid),
                    jobid,
                    callback); 
};
