...
Manager.prototype.submitJob = function submitJob(jobid, data, opts, submitted_cb, completed_cb) {
    var self = this;
    opts = opts || {};

    /*jshint evil:true*/
    self.redis.eval(scripts.addJob, 4, 
                    this.queuekey, this.delayqueuekey, this.delayprioritieskey, this.jobKey(jobid),
                    jobid, JSON.stringify(data), opts.priority || 0, Date.now(), opts.delay || 0, !!opts.nx ? 1:0, !!opts.autoremove ? 1 : 0,
                    function(err, result) {
        if(err) {
            return submitted_cb(err, result);
        } else {
            if(result === "1") {
                self.snoopJob(jobid, completed_cb);
...
Manager.prototype.snoopJobbyJob = function snoopJob(jobid, cb) {
    var self = this;
...
Manager.prototype.snoopJob = Manager.prototype.snoopJobbyJob;
...
Manager.prototype.removeJob = function removeJob(jobid, callback) {
    ...
    this.redis.eval(scripts.removeJob, 4,
                    this.queuekey, this.delayqueuekey, this.delayprioritieskeys, this.jobKey(jobid),
                    jobid,
                    callback); 
};
