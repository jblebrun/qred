...
Manager.prototype.removeJob = function(jobid, cb) {
    delete this.completion_callbacks[jobid];
    Qred.prototype.removeJob.call(this, jobid, cb);
};
...
Manager.prototype.snoopJobbyJob = function snoopJob(jobid, cb) {
    ...
    if(callbacks.length > 10) {
        self.log("CALLBACKS FOR "+self.name+":"+jobid+" LENGTH IS NOW: "+callbacks.length);
    }
};
