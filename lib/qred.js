...
var scripts = {
    ...
    getNextJob:""
        ...
        +"end "
        +"local jobid = redis.call('zrange', kQueue, 0, 1)[1] "
        +"redis.call('zremrangebyrank', kQueue, 0, 1) "
        //Return the next queued job
        +"if jobid then "
        +"  redis.log(redis.LOG_WARNING, 'Returning job: '..jobid) "
        +"  redis.log(jobid) "
        +"  local priority = redis.call('hget', kPriorityHash, jobid) "
        ...
        +"    local nextdelay = redis.pcall('zrange', kDelayQueue, 1, 'withscores')[2] "
