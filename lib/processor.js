...
Processor.prototype._process = function process() {
    ...
    self.active++;
    /*jshint evil:true*/
    this.redis.eval(scripts.getNextJob, 3,
                    this.queuekey, this.delayqueuekey,  this.delayprioritieskey,
                    Date.now(),
                    function(err, result) {
        self.active--;
        if(err) {
        ...
        if(result && result[0] === 1) {
                ...
                self.redis.hgetall(jobKey, function(err, job) {
                    if(err) {
                        self.log("Error retreiving next job");
                        self.log(err);
                        return;
                    }
                    var data = job.data;
                    ...
                    self.handler(data, function(err, result) {
                        job.autoremove = parseInt(job.autoremove, 10);
                        if(job.autoremove) {
                            self.redis.pexpire(jobKey, job.autoremove);
                        }
                        self.active--;
                        //Remove job?
