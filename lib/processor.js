...
Processor.prototype._process = function process() {
    ...
    this.redis.eval(scripts.getNextJob, 4,
                    this.queuekey, this.delayqueuekey, this.activequeuekey, this.delayprioritieskey,
                    Date.now(),
                    function(err, result) {
        self.active--;
        if(err) {
            ...
            self.clearProgress(jobid, function(err) {
                ...
                self.handler(data, function(job_err, result) {
                    ...
                    self.redis.eval(scripts.finishJob, 5,
                                    self.activequeuekey, self.completesetkey, self.livesetkey, self.delayqueuekey, jobKey, 
